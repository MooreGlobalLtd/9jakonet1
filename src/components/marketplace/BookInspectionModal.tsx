...
import { Wrench, ShieldCheck, MapPin, X, CheckCircle2, ArrowRight, Loader2, Search } from 'lucide-react';
...
export function getRecommendedTradeForItem(item?: MarketplaceItem | null): string {
  if (!item) return 'Electrician';
...
export default function BookInspectionModal({ isOpen, onClose, item, user }: BookInspectionModalProps) {
  ...
  // All hooks MUST be declared unconditionally at top level to satisfy React Rules of Hooks
  ...
  useEffect(() => {
    if (item) {
      ...
      setSelectedTrade(defaultTrade);
      setInspectionAddress(`${item.city || ''}${item.city && item.state ? ', ' : ''}${item.state || ''}`.trim());
      setNotes(`Inspect item "${item.title || ''}" for genuine parts, physical condition, and operational functionality before buyer payment.`);
      setBookedJobId(null);
      setFee(5000);
    }
  }, [item, isOpen]);
  ...
  // Conditional render ONLY AFTER all hooks are called
  if (!isOpen || !item) {
    return null;
  }

  const itemImage = (Array.isArray(item.images) && item.images[0]) || 
    ((item as any)?.photos && Array.isArray((item as any).photos) && (item as any).photos[0]) || 
    'https://via.placeholder.com/120x90';
  const itemPrice = Number(item.price) || 0;
  const itemLocation = `${item.city || ''}${item.city && item.state ? ', ' : ''}${item.state || 'Nigeria'}`.trim() || 'Nigeria';
  ...
  const handleSubmitBooking = async (e: React.FormEvent) => {
    ...
    try {
      const jobData = {
        ...
        customerPhone: phone || '',
        title: `Physical Pre-Purchase Inspection: ${item.title || 'Marketplace Item'}`,
        trade: selectedTrade,
        amount: fee,
        platformFee: 0,
        artisanPayout: fee,
      ...
      if (item.sellerId) {
        await sendInAppNotification({
          ...
          title: `Artisan Inspection Booked for "${item.title || 'your item'}"`,
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/65 z-50 flex items-center justify-center p-3 sm:p-4 backdrop-blur-xs overflow-y-auto">
      <div className="bg-white rounded-2xl max-w-xl w-full max-h-[94vh] overflow-y-auto shadow-2xl border border-slate-200 animate-in fade-in zoom-in-95 duration-200">
        ...
        {bookedJobId ? (
          <div className="p-6 text-center space-y-4">
            ...
            <p className="text-sm text-slate-600 max-w-md mx-auto leading-relaxed">
              Your inspection request for <strong>{item.title}</strong> has been created. A verified <strong>{selectedTrade}</strong> in {item.state || 'your area'} will inspect condition and send you a photo/diagnostic report before you complete payment.
            </p>
            <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-3.5 text-xs text-emerald-900 flex items-center gap-2 justify-center">
              ...
              <span>Inspection fee (₦{(fee || 5000).toLocaleString()}) is held in 9jaKonet Escrow.</span>
            </div>
            ...
            <div className="pt-3 flex flex-col sm:flex-row gap-2.5 justify-center">
              <Button
                ...
                onClick={() => {
                  onClose();
                  navigate(`/explore?category=${encodeURIComponent(selectedTrade)}&state=${encodeURIComponent(item.state || 'All Nigeria')}`);
                }}
              ...
              >
                ...
                Browse {selectedTrade}s in {item.state || 'Nigeria'}
              </Button>
              <Button
                type="button"
                onClick={() => {
          ...
          <form onSubmit={handleSubmitBooking} className="p-5 sm:p-6 space-y-4">
            {/* Item Card Summary */}
            <div className="flex gap-3.5 bg-slate-50 p-3.5 rounded-xl border border-slate-200">
              ...
              <div className="min-w-0 flex-1">
                ...
                <h4 className="text-xs sm:text-sm font-bold text-slate-900 truncate">{item.title || 'Untitled Listing'}</h4>
                <p className="text-sm font-extrabold text-emerald-700 mt-0.5">₦{itemPrice.toLocaleString()}</p>
            ...
            <div className="bg-slate-50 border border-slate-200 rounded-xl p-3.5 flex items-center justify-between">
              ...
              <div className="flex items-center gap-1">
                ...
                <input
                  ...
                  step="500"
                  value={fee}
                  onChange={(e) => setFee(Number(e.target.value) || 2000)}
            ...
            <div className="pt-2 space-y-2">
              ...
              >
                {isSubmitting ? (
                  ...
                  <>
                    ...
                    Book Inspection with Escrow (₦{(fee || 5000).toLocaleString()})
                  </>
                )}
              </Button>
              ...
              >
                <span>Or view all rated {selectedTrade}s in {item.state || 'Nigeria'}</span>
