...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (data.status && data.data) {
      ...
      return res.status(400).json({ success: false, error: data.message || 'Failed to execute transfer' });
    }
  } catch (error: any) {
    return res.status(500).json({ success: false, error: error?.message || 'Server error executing transfer' });
  }
}
